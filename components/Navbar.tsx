...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[120] bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-900 px-6 h-20 flex items-center transition-colors duration-300">
      <div className="max-w-7xl mx-auto w-full flex items-center justify-between relative">
        ...
        <form onSubmit={handleSubmit} className="flex-1 max-w-xl mx-8 relative">
          <input 
            ...
            className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full py-2.5 pl-10 pr-4 text-sm outline-none dark:text-white focus:ring-2 focus:ring-[#049454]/20 transition-all"
          />
          <div className="absolute left-3.5 top-1/2 -translate-y-1/2">
            {isSearching ? <div className="w-4 h-4 border-2 border-[#049454] border-t-transparent rounded-full animate-spin"></div> : <svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>}
          </div>
        </form>
